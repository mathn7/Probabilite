...
figure('Name','Histogramme des differences de niveaux de gris','Position',[0.33*L,0,0.33*L,0.3*L]);
plot([I_min:I_max],histogramme);
axis([I_min I_max 0 1.1*max(histogramme)]);
xlabel('Ecart entre niveaux de gris voisins','FontSize',15);
...
figure('Name','Mise en evidence de la decorrelation entre pixels voisins','Position',[0.66*L,0,0.33*L,0.3*L]);
plot(X,Y,'b+');
